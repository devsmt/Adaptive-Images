...
class Image {
    ...
    public static function cacheRefresh($source_file, $cache_file, $resolution) {
        ...
        return Image::generate($source_file, $cache_file, $resolution);
    }


    // helper function: Send headers and returns an image.
    public static function send($filename, $browser_cache) {
    ...
    public static function generate($source_file, $cache_file, $resolution) {
        ...
        // sharpen the image?
        if ($sharpen == TRUE) {
            $intSharpness = Image::findSharp($width, $new_width);
            $arrMatrix = array(
                array(-1, -2, -1),
        ...
        if (!is_dir($cache_dir)) {
            if (!mkdir($cache_dir, 0777, true)) {
                ...
                if (!is_dir($cache_dir)) {
                    ...
                    ImageDestroy($dst);
                    Image::sendError("Failed to create directory: $cache_dir");
                }
            }
        }

        if (!is_writable($cache_dir)) {
            Image::sendError("The cache directory is not writable: $cache_dir");
        }
        ...
        if (!$gotSaved && !file_exists($cache_file)) {
            Image::sendError("Failed to create image: $cache_file");
        }
...
if (!extension_loaded('gd')) { // it's not loaded
    if (!function_exists('dl') || !dl('gd.so')) { // and we can't load it either
        ...
        Image::send($source_file, $browser_cache);
    }
}

// Do we need to switch mobile first off?
if(Browser::detect() == "gecko"){
    $mobile_first = FALSE;
}

$resolution = Main::getResolution();
...
if (file_exists($cache_file)) { // it exists cached at that size
    ...
    //     $cache_file = Image::cacheRefresh($source_file, $cache_file, $resolution);
    // }
